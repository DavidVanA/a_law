...
int a_law_decomp(WAV_Header *header, FILE *input, FILE *output)
{
	...
	uint8_t sample;		// Sample to be converted
	uint32_t length = 0;	// Total length of data
	int16_t converted;	// Converted value to be written
	...
	for(int i = 0; i < chunks; i++) {
		...
		// Shift over by number of bits needed to get next sample
		converted = a_law_unconvert(sample);
		// Write new data to output file
		fwrite(&converted, 2, 1, output);
		// Increment number of bytes written
		length++;
	}

	// Length is 2* since 16-bit data
	length *= 2;
...
int16_t a_law_unconvert(uint8_t val)
{
	...
	int16_t val_signless, chord, step;
    	uint16_t result;

	// Get value without sign
    	val_signless = val & (0x007F);

	// Extract chord
    	chord = val_signless >> 4;
	// Extract step
    	step = val_signless & (0x000F);

	// If the chord was not zero, add leading 1
	if(chord > 0)
		step += 16;
	...
	// Shift by (chord - 1) if chord greater than 1
	if(chord > 1)
		result = result << (chord-1);

	// Invert if greater than 127
	register uint32_t val_long = val;
	if(val > 127) return result;
	else return -result;
}
